...
namespace Nyx.Network.ServerSocket.AuthServer
{
    public abstract class AuthServerSocket
    {
        ...
        public new void Enable(string _IPAddress, ushort Port)
        {
            try
            {
                ...
                _listener.Bind(new IPEndPoint(IPAddress.Parse(_IPAddress), Port));
                _listener.Listen(MaxConcurrentConnections);

                LoggingService.SystemInfo("Network", $"Improved server socket enabled on {_IPAddress}:{Port}");
        ...
        public void Reset(string IpAddress, ushort Port)
        {
            Disable();
            Enable(IpAddress, Port);
        }
