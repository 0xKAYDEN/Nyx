using System;
using System.Text;
namespace Nyx.Server.Network.AuthPackets
{
    public class Forward : Interfaces.IPacket
    {
        public enum ForwardType : byte { Ready = 2, InvalidInfo = 1, Banned = 25 }
        byte[] Buffer;
        public Forward()
        {
            Buffer = new byte[52];
            Network.SafeWriter.Write(52, 0, Buffer);
            Network.SafeWriter.Write(0x41f, 2, Buffer);
        }
        public uint Identifier
        {
            get
            {             
                return BitConverter.ToUInt32(Buffer, 4);
            }
            set
            {
                Network.SafeWriter.Write(value, 4, Buffer);
            }
        }
        public ForwardType Type
        {
            get
            {
                return (ForwardType)(byte)BitConverter.ToUInt32(Buffer, 8);
            }
            set
            {
                Network.SafeWriter.Write((byte)value, 8, Buffer);
            }
        }
        public string IP
        {
            get
            {
                return Encoding.Default.GetString(Buffer, 20, 16);
            }
            set
            {
                Network.SafeWriter.Write(value, 20, Buffer);
            }
        }
        public ushort Port
        {
            get
            {
                return BitConverter.ToUInt16(Buffer, 12);
            }
            set
            {
                Network.SafeWriter.Write(value, 12, Buffer);
            }
        }
        public void Deserialize(byte[] buffer)
        {
            //no implementation
        }
        public byte[] Encode()
        {
            return Buffer;
        }
        public void Send(Client.GameClient client)
        {
            client.Send(Buffer);
        }
    }
}