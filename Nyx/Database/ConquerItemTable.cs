        ...
        public static void AddItem(ref ConquerItem Item, Client.GameClient client)
        {
            try
            {
                using (var cmd = new MySqlCommand(MySqlCommandType.INSERT).Insert("items"))
                    cmd.Insert("ID", Item.ID).Insert("UID", (uint)(Item.UID == 0 ? ConquerItem.ItemUID.Next : Item.UID))
                        ...
                        .Insert("Bound", Item.Bound).Insert("Locked", Item.Lock).Insert("UnlockEnd", Item.UnlockEnd.Ticks).Insert("TimeStamp", Item.TimeStamp.Ticks).Insert("MinutesLeft", Item.Minutes)
